...
public class XRCPProjectile1 : MonoBehaviour
{
    ...
    void HandleProjectile(GameObject projectile)
    {
    ...
    IEnumerator MoveProjectile(GameObject projectile)
    {
        ...
        while (true)
        {
            // 如果投射物的Y坐标超过某个阈值，停止运动
            if (projectile.transform.position.y > 10.0f)
            {
                rigidbody.velocity = Vector3.zero;
                rigidbody.isKinematic = true; // 停用刚体运动
                break;
            }
